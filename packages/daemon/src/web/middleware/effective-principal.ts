...
import { eventChannelKind } from "../../lib/chat/system-events.js";
 ...
 *              daemon-spawned script). What it clears is deliberately narrow: the
 *              extension layer's coordinator gates (`isPrivileged` — intention
 *              review, acting `--mind` in commands). It does NOT clear
 *              `requireAdmin` — the spirit configures its own schedules, so
 *              anything gated only on `system` is, for the spirit, gated on
 *              nothing — and it does not widen `requireSelf`: the spirit's
 *              cross-mind route reach stays exactly the `requireSelfOrSpirit`
 *              allowlist (#1020), whatever tier the request carries.
 * - `basic`  — least privilege. The spirit with no verifiable requesting
 *              principal: it can still act on itself (and use its allowlist),
 *              and nothing else.
 */
...
/**
 * Admin-equivalent-or-self-initiated authority: a real admin, the spirit acting for a
 * verified admin, or the spirit's own daemon-evidenced work. This is the extension
 * layer's coordinator gate (`ctx.isPrivileged` / command `ctx.privileged`) — the tier
 * check that keeps the spirit's *scheduled* tending (intention review, acting `--mind`)
 * working. Daemon privilege sites use {@link hasAdminAuthority} instead: #1020 decided
 * per-site that the spirit's self-initiated work does not need them.
 */
export function hasSystemAuthority(e: Effective | undefined): boolean {
  return e?.role === "admin" || e?.role === "system";
}

/** Strict admin authority. The spirit's self-initiated work is deliberately excluded. */
 ...
 * mind can send text" to admin-equivalent API access (#433). #1020 removed the
 * standing power; this restores the *affordance* that went with it: the spirit
 * borrows the authority of whoever triggered the turn its request arrives in —
 * capped at that principal's real role, so nobody gains anything by asking the
 * spirit instead of asking directly.
 *
...
export async function resolveEffective(principal: {
  ...
  if (inbound.length === 0) {
    const kinds = rows.map((r) => eventChannelKind(r.channel));
  ...
  // privilege for exactly that reason. This must stay a SEPARATE condition from the
  // slug comparison below, not be folded into it: a channel *named after* an admin has
  // slug `#james` where their DM has `@james` — one character the sender comparison
  // never sees — so with only the slug check, anyone who can name a channel could mint
  // a room that lends its namesake's authority to whoever speaks in it.
  ...
  // a users row so it can appear in conversations, not so it can lend authority. This
  // check is load-bearing on its own, not a belt for the slug check below: a puppet's
  // namespaced username (`discord:99999`, #1019) slugifies to the same string as its
  // own DM channel slug (`@discord-99999` — slugify maps `:` to `-`), so a puppet's
  // sender and channel *agree* and only this condition refuses it.
  ...
  // A Discord user who renames themselves `james` produces a row whose channel is
  // `@discord-12345` — the puppet they actually are. Requiring the two to agree is what
  // stops a rename outside this system from becoming admin inside it.
  //
  ...
  // slug could still pass. Namespacing external senders at the point of record (#1019)
