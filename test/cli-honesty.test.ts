...
describe("resolveActingMind (#907)", () => {
  ...
  // `--mind` is an impersonation flag, and the spirit is reachable by everyone — so a
  // spirit privileged on identity would let any mind that talked it into `--mind <admin>`
  // reach that admin's extension data, and sail through a downstream `role === "admin"`
  // check on the impersonated identity (#433). `privileged` is therefore the request's
  // resolved authority, and false is what a spirit turn triggered by a mind's DM
  // resolves to — it must be refused like any other unprivileged caller.
