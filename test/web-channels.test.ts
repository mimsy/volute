...
describe("web channels routes", () => {
  ...
  it("POST /:name/channels/send — 404 for nonexistent agent", async () => {
    ...
    const res = await app.request(
      "http://localhost/api/agents/nonexistent-ch-agent/channels/send",
      {
        method: "POST",
  ...
  it("POST /:name/channels/send — 400 for invalid platform", async () => {
    const cookie = await setupAuth();
    addAgent(TEST_AGENT, 4160);
    const { default: app } = await import("../src/web/app.js");

    const res = await app.request(`http://localhost/api/agents/${TEST_AGENT}/channels/send`, {
      method: "POST",
      headers: postHeaders(cookie),
  ...
  it("GET /:name/channels/read — 404 for nonexistent agent", async () => {
    ...
    const res = await app.request(
      "/api/agents/nonexistent-ch-agent/channels/read?platform=discord&uri=test:chan",
  ...
  it("GET /:name/channels/read — 400 when missing platform or uri", async () => {
    ...
    const res = await app.request(`/api/agents/${TEST_AGENT}/channels/read`, {
      headers: getHeaders(cookie),
    });
    assert.equal(res.status, 400);
    const body = (await res.json()) as { error: string };
  ...
  it("GET /:name/channels/read — 400 for invalid platform", async () => {
    ...
    const res = await app.request(
      `/api/agents/${TEST_AGENT}/channels/read?platform=bogus&uri=test`,
      { headers: getHeaders(cookie) },
    );
    assert.equal(res.status, 400);
    const body = (await res.json()) as { error: string };
  ...
  it("GET /:name/channels/list — 404 for nonexistent agent", async () => {
    ...
    const res = await app.request("/api/agents/nonexistent-ch-agent/channels/list", {
      headers: getHeaders(cookie),
    });
    assert.equal(res.status, 404);
  });
  ...
  it("GET /:name/channels/list — returns object keyed by platform", async () => {
    ...
    const res = await app.request(`/api/agents/${TEST_AGENT}/channels/list`, {
      headers: getHeaders(cookie),
    });
  ...
  it("GET /:name/channels/users — 404 for nonexistent agent", async () => {
    ...
    const res = await app.request(
      "/api/agents/nonexistent-ch-agent/channels/users?platform=discord",
      { headers: getHeaders(cookie) },
    );
    assert.equal(res.status, 404);
  });
  ...
  it("GET /:name/channels/users — 400 when missing platform", async () => {
    ...
    const res = await app.request(`/api/agents/${TEST_AGENT}/channels/users`, {
      headers: getHeaders(cookie),
    });
    assert.equal(res.status, 400);
    const body = (await res.json()) as { error: string };
  ...
  it("GET /:name/channels/users — 400 for platform without listUsers", async () => {
    ...
    const res = await app.request(`/api/agents/${TEST_AGENT}/channels/users?platform=system`, {
      headers: getHeaders(cookie),
    });
    assert.equal(res.status, 400);
    const body = (await res.json()) as { error: string };
  ...
  it("POST /:name/channels/create — 404 for nonexistent agent", async () => {
    ...
    const res = await app.request(
      "http://localhost/api/agents/nonexistent-ch-agent/channels/create",
      {
        method: "POST",
        headers: postHeaders(cookie),
  ...
  it("POST /:name/channels/create — 400 for platform without createConversation", async () => {
    ...
    const res = await app.request(`http://localhost/api/agents/${TEST_AGENT}/channels/create`, {
      method: "POST",
      headers: postHeaders(cookie),
  ...
  it("GET /:name/channels/read — requires auth (401 without cookie)", async () => {
    ...
    const res = await app.request(
      `/api/agents/${TEST_AGENT}/channels/read?platform=discord&uri=test`,
    );
    assert.equal(res.status, 401);
  });
});
