...
const TEST_USERNAMES = [SPIRIT, OTHER_MIND, ASKING_MIND, ADMIN, PLAIN, PENDING];
...
describe("spirit effective principal (#433)", () => {
  ...
  it("keeps webhook out of the self-initiated allowlist and schedule in it", () => {
    // The allowlist is the security boundary, so pin both halves: a kind that drops
    // out silently breaks tending, a kind that creeps in silently reopens #433.
    assert.ok(_selfInitiatedEventKinds.has("schedule"));
    assert.ok(_selfInitiatedEventKinds.has("wake"));
    assert.ok(!_selfInitiatedEventKinds.has("webhook"));
  });
