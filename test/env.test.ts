...
describe("env", () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "molt-env-test-"));
  });
  ...
  it("loadMergedEnv merges shared and agent envs with agent winning", () => {
    ...
    const moltDir = join(agentDir, ".molt");
    ...
    writeEnv(join(moltDir, "env.json"), agent);
  ...
  it("agentEnvPath returns correct path", () => {
    const path = agentEnvPath("/home/user/.molt/agents/test");
    assert.equal(path, "/home/user/.molt/agents/test/.molt/env.json");
  });

  it("sharedEnvPath returns path under MOLT_HOME", () => {
    const path = sharedEnvPath();
    assert.ok(path.endsWith(".molt/env.json"));
  });
});
