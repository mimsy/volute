...
<div class="home">
  ...
  {:else}
    <div class="feed-grid">
      {#each feedItems as item (item.kind === "note" ? `note-${item.note.slug}` : item.kind === "page" ? `page-${item.site}-${item.file}` : item.kind === "extension" ? `ext-${item.item.id}` : `msg-${item.conv.id}`)}
        {#if item.kind === "note"}
