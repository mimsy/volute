...
- **Identity** (SOUL.md, MEMORY.md, `memory/`, `.config/`, the mechanics doc) — the mind's own. Never re-added, never overwritten, not even when missing. One narrow exception: when a mechanics-doc paragraph Volute wrote stops being true, `lib/mind/mechanics-doc.ts` replaces that paragraph on upgrade only while it is still byte-identical to a shipped wording, and tells the mind once when it has reworded it instead (#1144). Changing such a paragraph in a template means moving the outgoing text into that module's `stale` list — `test/mechanics-doc-repair.test.ts` pins it.
- **Infrastructure** (`.local/**` — hooks and bin shims) — Volute's machinery namespace, which the daemon itself generates into and executes. Upgrades **add** any of these the mind is missing via `backfillInitInfrastructure()`, and never overwrite one that is present (a mind may have edited its own hook) — except to **refresh** a file still byte-identical to a version Volute shipped, per the `.init/.local/SHIPPED.json` hash ledger.

  A mind's *deletion* of an infrastructure file is authorship too, and is honoured: `lib/mind/init-ledger.ts` keeps a per-mind ledger at `stateDir(<name>)/init-infrastructure.json` of every infrastructure path the mind has ever been given, and the backfill **withholds** a path that is in the ledger and absent from disk (#811). Not-in-ledger-and-absent is the #808 case and is still added. The ledger is seeded at creation (`seedInitLedger()` on `applyInitFiles()`'s return) and maintained by the backfill itself — every path observed present or successfully installed is recorded, so a mind predating the ledger derives one from disk on its first run and a removal made before that ships is undone exactly once. Never write a ledger entry for a file that did not land: in-ledger + absent withholds it forever. `backfillInitInfrastructure`'s `honorRemovals: false` exists for the one caller that knows an absence is its own doing — an upgrade whose `restoreMergeDeletedHomeFiles` threw partway — since reading that damage as authorship would withhold the files silently and permanently.

  A **home-only** export/import moves both the mind's edits and its removals, and needs three things to do it (#1013): `home/.local/` in the archive (git never reports it — `templates/_base/gitignore` ignores `home/*` — so `listHomeFiles()` walks the subtree in explicitly, and the non-git fallback goes through the same helper); the exporting host's ledger at `state/init-infrastructure.json` in the zip, which is the only thing that distinguishes "the mind deleted this hook" from "this hook shipped after the export" (#808); and, on import, `overlayArchiveHome()` removing exactly the freshly-composed paths that ledger recorded as given and the archive does not carry, with the new ledger seeded from the union of both hosts' records. The carried ledger is untrusted archive content — contain each path *before* judging whether it is infrastructure, or `.local/../x` reads as in-namespace. Full-archive export is unaffected.

The rule is the `.local/` subtree, not a filename list, so a hook added under `.local/hooks/` is covered the day it ships. `test/template-init-classification.test.ts` pins the classification of every shipped `.init/` file, so any newly shipped `.init/` file fails CI until it is classified on purpose. Skipping the backfill is how a capability shipped as a new hook reaches only minds created after it existed while the daemon-side half looks healthy (#808 — the notices drain hook, the sole reader of next-turn system events).
...
- **Authorize on the request's *effective* authority, never on the caller's stored role.** `c.get("effective")` (`packages/daemon/src/web/middleware/effective-principal.ts`) is what the guards read; extensions get the same answer from `ctx.isPrivileged(c)`. It matters for exactly one principal, and that principal is the reason the rule exists: the spirit authenticates as the shared system user, and *anyone can talk to it*, so a role check hands admin-equivalent access to whatever a mind talked it into (#433). Its authority is instead resolved per-request from the daemon's own turn tracker and the senders the daemon recorded on that turn — never from the session slug it self-reports — and every unresolvable case falls to `basic`. Use `hasAdminAuthority` for daemon privilege sites (a real admin, or the spirit on a verified admin's turn) and `hasSystemAuthority` for extension coordinator gates (those two, plus the spirit's own daemon-evidenced schedule/script work); a new gate written as `role === "admin" || role === "spirit"` reopens the hole, and `test/authz-coverage.test.ts` fails CI on spirit-identity grants.
...
- **Contain attacker-controllable filesystem paths.** Any fs operation (read/write/delete) whose path includes a request value, mind name, filename, or config field (e.g. `profile.avatar`) must go through `resolveWithinBase()` / `safeResolveWithinBase()` in `packages/daemon/src/lib/util/paths.ts`, or be reduced to a `basename()`. `resolve(base, userPath)` alone does **not** prevent `../` or absolute-path escape. Remember fs operations in `web/api/*` run with **daemon** privileges, not the mind's sandbox.
  - Lexical containment is not enough inside a mind's own tree: the mind can plant a symlink, hard link or FIFO at any component, and the daemon is root under user isolation. Read and write a mind's files through `writeMindFile` / `readMindFile` / `rewriteMindFileInPlace` (`packages/daemon/src/lib/mind/mind-file-write.ts`), passing `mindFileOwner(baseName)` as the owner — never a bare `writeFile`/`readFile`/`mkdir -p` on a path under a mind dir (#1110, #1123).
...
- **A child never inherits the daemon environment.** `exec`/`gitExec` lay the caller's `env` over `buildMindBaseEnv()` (`lib/util/mind-env.ts`), an allowlist without `VOLUTE_DAEMON_TOKEN`. The default is the fix, not the call sites: `env: undefined` means *inherit everything* to `execFile`, so a `gitExec` that passed no env handed the admin token to whatever `core.hooksPath` a mind pointed at (#966). Add what a child needs one variable at a time; a caller that may run mind-authored code never passes `...process.env` (`backup/restic.ts` is the documented exception — a host binary that needs the host's own store credentials). `exec` also deletes `VOLUTE_DAEMON_TOKEN` from the merged env unconditionally, so a stray spread cannot re-admit the one secret that matters. Spawning outside the wrappers means passing `buildMindBaseEnv()` yourself, as `mind/spawn-server.ts` and the pages extension's private git runner do — a bare `spawn(cmd, args, { cwd })` inherits everything, and the isolation wrap is not a substitute (`runuser` passes the environment through, and under sandbox/none there is no uid change at all). Two exits exist, both deliberate and both documented where they live: `execInherit`, for host-facing service and CLI commands only and never a command in a mind's repo, and the allowlisted spreads named in the scan below. `test/exec-env.test.ts` pins all of it — a planted pre-commit hook that records what it saw, plus a scan that fails CI on a new `...process.env` spread under `lib/` or `packages/extensions/` outside its documented allowlist.
...
- **`user`** — System installs create per-mind OS users (`mind-<name>`, prefix configurable via `VOLUTE_USER_PREFIX`). On Linux, uses `useradd`/`runuser`; on macOS, uses `dscl`/`sudo -u`. Mind and bridge processes spawn with the mind's uid/gid. Requires root. `ensureMindUser()` in `isolation.ts` self-heals a mind whose OS user has vanished while its files remain (a recreated Docker container keeps `/data` and `/minds` but not `/etc/passwd`), recreating it on the uid that owns its directory (plus the gid on Linux, where mind dirs are `<user>:<user>`; macOS reuses only the uid, since its mind dirs are `<user>:volute`). It refuses — rather than substituting a fresh id — when the dir is root-owned or the id belongs to another account, because in the wipe scenario every mind's uid is free in passwd while still owning files, so a fresh id can land on another mind's directory. A refusal goes out through `alertHost()` once per daemon run — to the mind, the spirit, and the dashboard — carrying the exact commands a host should run; the once-per-run gate exists because refusals are deliberately not memoised, so a host's manual fix is noticed on the next call. It is wired into `wrapForIsolation`, `chownMindDir` and `chownMindFile`, and is best-effort — it never throws over the caller's own error.
