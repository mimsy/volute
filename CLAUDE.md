...
- **Never expose secrets over the API to non-owners.** Env values, tokens, and keys must be `requireAdmin`/`requireSelf`-gated. The sandbox blocks on-disk reads of `env.json`/`volute.db`; don't let an API endpoint hand the same data back.
